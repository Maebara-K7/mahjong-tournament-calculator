...
6.  `saikouisen_quiz.py`: **最高位战规则练习工具**。专门用于练习最高位战（Saikouisen）规则下的终局条件计算。
...
## 最高位战规则练习 (`saikouisen_quiz.py`)

除了通用的练习工具 `practice.py`，本项目还包含一个专门针对**日本最高位战（Saikouisen）规则**的练习脚本 `saikouisen_quiz.py`。

该脚本旨在模拟最高位战独特的、多半庄累计积分的竞赛环境，并生成与之相关的复杂终局情景。

### 功能与特点

- **真实情景模拟**: 能够生成包含前几个半庄累计得分的复杂场景，使练习更贴近真实比赛。
- **智能题库生成**: 在练习过程中，脚本会自动筛选出难度（Hard）与质量（Good）俱佳的题目，并将其保存到 `saikouisen_quiz_archive.json` 文件中。
- **练习与考试模式**:
    - **练习模式 (默认)**: 直接运行 `python saikouisen_quiz.py`，程序会不断生成新的随机场景供您练习。
    - **考试模式**: 在程序启动后，输入 `2` 即可进入考试模式。该模式会从 `saikouisen_quiz_archive.json` 题库中随机抽取题目进行测验。

### 如何使用

```bash
python saikouisen_quiz.py
```

- 直接开始练习，程序将自动生成题目。
- 如果想进入考试模式，请在程序启动后根据提示操作。